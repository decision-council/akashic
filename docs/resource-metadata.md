...
`programChecked` is narrower than `reviewed`: it records when a curator checked primary program pages for operational facts such as eligibility, application cadence, costs, and obligations. It is not a human truth review, endorsement, eligibility determination, or promise that a call remains open. The Open Source Sustainability and Support and Financial Relief and Assistance collections require it because these facts change materially between calls or applications. Financial-relief entries also follow the [monthly review and visible eligibility convention](financial-relief-curation.md).
