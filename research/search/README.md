...
The 2026-09-17 corpus refresh records 5,257 resources, up from the previous 5,112-resource checkpoint. It accompanies the generated-guide payload change in #113. The v1 fixture, algorithms, grades, and acceptance gate are unchanged; the v2 tax-help judgment follows the existing VITA resource alias to its canonical IRS URL after the earlier collection migration. The active scorer produces identical v2 summary metrics and top-ten results on this PR and its 5,251-resource `main` baseline. Differences from the older committed checkpoint reflect intervening corpus changes, not a new scoring algorithm. Historical reports remain available in Git history.
...
- records mean Recall@10 of `0.0972` and mean reciprocal rank of `0.0842`; and
...
- records mean Recall@10 of `0.5694` and mean reciprocal rank of `0.6173`; and
- surfaces twelve of nineteen explicitly judged known-irrelevant URL-query pairs in the top ten.
...
The evaluator also ranks all 36 generated subqueries independently to depth ten and deduplicates their candidates with the original top ten. On the expanded seed, decomposition produces a mean candidate pool of `13.2222` resources—`4.7778` additional candidates per case. It adds one judged-relevant candidate for the uninsured-doctor query and one known negative for `AO 240`. That creates measurable fusion headroom, but one positive case does not justify a portal ranking change; decomposition still does not change result order.
...
- frozen `and-substring-v1` finds a judged-relevant result in four cases, with mean Recall@10 `0.0859`, MRR `0.0786`, and nDCG@10 `0.0699`;
- active `weighted-lexical-v2` succeeds in eighteen cases, with mean Recall@10 `0.4948`, MRR `0.4632`, and nDCG@10 `0.4362`;
- the active scorer surfaces 23 of 34 deliberately selected known-negative probes in the top ten, showing substantially more over-broad-result evidence than v1 exposed; and
...
The unchanged `fusion-no-harm-v1` gate still rejects `decomposition-rrf-v1` on v2. Fusion gains one top-ten success and lowers known-negative exposure by one, but MRR falls by `0.0204`, nDCG@10 falls by `0.0157`, six first-relevant ranks regress, one exact-identifier rank regresses, and the federal fee-waiver case still gains a known negative. `weighted-lexical-v2` therefore remains the portal ranking.
...
Evaluation v2 broadens the timed query distribution from 18 to 32 catalog-covered cases without changing the deployed scorer. The JIT-disabled median ceiling is therefore rebaselined from `250` to `300` milliseconds; its `800`-millisecond p95 and `1200`-millisecond single-query ceilings remain unchanged so CI still catches tail regressions.
