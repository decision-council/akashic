...
The 2026-09-17 corpus refresh for #137 records 5,301 resources, up from the 5,257-resource checkpoint accompanying #113. It is required by the compatible generated-catalog serialization change; both fixtures, algorithms, judgments, acceptance gate, and performance limits remain unchanged. Against the merged 5,292-resource parent, all 33 active v2 top-ten result objects and retrieval-quality summary metrics are identical. Total match counts, some ranks outside the top ten, and decomposition candidate-pool diagnostics change as the catalog grows. Differences from the older committed checkpoint also include intervening collection changes. Historical reports remain available in Git history.
...
- active `weighted-lexical-v2` succeeds in eighteen cases, with mean Recall@10 `0.4948`, MRR `0.4631`, and nDCG@10 `0.4362`;
...
The unchanged `fusion-no-harm-v1` gate still rejects `decomposition-rrf-v1` on v2. Fusion gains one top-ten success and lowers known-negative exposure by one, but MRR falls by `0.0203`, nDCG@10 falls by `0.0157`, six first-relevant ranks regress, one exact-identifier rank regresses, and the federal fee-waiver case still gains a known negative. `weighted-lexical-v2` therefore remains the portal ranking.
