...
import { parseRootCategories, serializeCatalog, serializeCompactCatalog } from "./lib/catalog.mjs";
...
async function build() {
  ...
  // Keep the v2 URL for cached clients and external consumers.
  await writeFile(path.join(outputDirectory, "data", "catalog.json"), serializeCatalog(catalog));
  await writeFile(path.join(outputDirectory, "data", "catalog-v3.json"), serializeCompactCatalog(catalog));
